...
import { addTask, toggleTaskDone } from "@/app/objects/actions"
...
export default function ObjectTasks({ objectId, tasks }: Props) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        <form action={addTask.bind(null, objectId)} className="flex flex-wrap gap-2">
          <input
            name="title"
        ...
        {tasks.length === 0 ? (
          <p className="text-center text-muted-foreground italic py-8">No tasks yet</p>
        ) : (
          <div className="space-y-3">
            {tasks.map((task) => {
              const nextDue = task.next_due_date ? new Date(task.next_due_date) : null
              const isOverdue = nextDue ? nextDue < new Date() && !task.is_done : false
              const dueText = nextDue
                ? isOverdue
                  ? `Overdue ${formatDistanceToNow(nextDue, { addSuffix: true }).replace(" ago", "")}`
                  : formatDistanceToNow(nextDue, { addSuffix: true }).replace("in ", "")
                : null
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs">
                        ...
                        {dueText && (
                          <Badge variant={isOverdue ? "destructive" : "outline"}>
                            ...
                            {isOverdue ? dueText : `Next ${dueText}`}
                          </Badge>
                        )}
                      </div>
                    </div>
